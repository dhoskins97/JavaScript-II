...
let allCaps = runners.map(function (elem) {
...
let largeShirts = runners.filter(function(elem) {
...
let ticketPriceTotal = runners.reduce(function(total, elem) {
...
// Problem 1: The director is awarding medals to those who donated over 200, return info about people who donated over 200.

let overAchievers = runners.filter(function (elem){
return elem.donation >= 200
})
console.log(overAchievers)

// Problem 2: The director is giving prices to the 35th person to sign up. Return an array containing the information of the 35th id-holder in the given array.

let luckyRunner = runners.filter(function (elem) {
return elem.id === 35
})
console.log(luckyRunner)

// Problem 3: The director really wants to see the amount of donations from each company. Return an array containing companies with their donation amounts.

let greedArray = [];
runners.forEach(function (elem) {
greedArray.push(`${elem.company_name} donated $${elem.donation}`)
})
console.log(greedArray)